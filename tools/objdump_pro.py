...
def clean_inst_args(inst, args):
	...
	if (
		inst not in ["sllv", "srlv", "srav", "mult", "multu", "div", "divu"]
		and len(args) >= 2
	):
		if args[0] == args[1]:
			del args[1]
...
def main(args):
	...
	data_inst_split = util.find_index(
		lambda line: line.data >> 0x18 not in [0x00, 0x80], output
	)
	inst_data_split = data_inst_split + util.find_index(
		lambda line: len(line.inst) == 0 or line.inst.startswith("0x"),
		output[data_inst_split:],
	)
...
""")
	...
	for line in output[:data_inst_split]:
		output_asm_file.write(
			f".L0x{line.addr:08x}: {line.to_data()} # {line.to_inst()}\n"
		)

	output_asm_file.write("# Start of code\n")
	for line in output[data_inst_split:inst_data_split]:
		output_asm_file.write(
			f".L0x{line.addr:08x}: {line.to_inst(): <35} # {line.to_data()}\n"
		)
	...
	if remaining_bytes is not None:
		for remaining_idx, remaining_byte in enumerate(remaining_bytes):
			output_asm_file.write(
				f".L0x{remaining_addr + 0x4 * remaining_idx:08x}: .byte 0x{remaining_byte:x}\n"
			)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--output-raw-exe-toml", dest="output_raw_exe_toml", type=str, required=True
	)
	parser.add_argument(
		"--output-elf-toml", dest="output_elf_toml", type=str, required=True
	)
